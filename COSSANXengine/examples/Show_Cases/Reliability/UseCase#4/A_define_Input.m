...
for i=1:6 % number of floor
    for j=1:16 %number of column
        ...
        Cmem_sections{96+(i-1)*16+j}=['sectionY_Col' int2str(j) '_Floor_' int2str(i)]; 
...
for i=1:6
    ...
    % the 6 floors.
    Cmem_Youngs{6+i}=['Youngs_Floor_' int2str(i)]; 
...
for i=1:6
    ...
    % the 6 floors.
    Cmem_Youngs{12+i}=['Youngs_Stairs_' int2str(i)]; 
...
for i=1:6
    ...
    % floors.
    Cmem_Density{6+i}=['Density_Floor_' int2str(i)]; 
...
for i=1:6
    ...
    % floors.
    Cmem_Density{12+i}=['Density_Stairs_' int2str(i)]; 
...
for i=1:6
    ...
    % the 6 floors.
    Cmem_Poisson{6+i}=['Poisson_Stairs_' int2str(i)];
...
%% Define Function objects for the shear moduli

Shear_Cols_Floor_1 = Function('Sexpression','<&Youngs_Cols_Floor_1&>./(2*(1+ <&Poisson_Floor_1&>))');
Shear_Cols_Floor_2 = Function('Sexpression','<&Youngs_Cols_Floor_2&>./(2*(1+ <&Poisson_Floor_2&>))');
Shear_Cols_Floor_3 = Function('Sexpression','<&Youngs_Cols_Floor_3&>./(2*(1+ <&Poisson_Floor_3&>))');
Shear_Cols_Floor_4 = Function('Sexpression','<&Youngs_Cols_Floor_4&>./(2*(1+ <&Poisson_Floor_4&>))');
Shear_Cols_Floor_5 = Function('Sexpression','<&Youngs_Cols_Floor_5&>./(2*(1+ <&Poisson_Floor_5&>))');
Shear_Cols_Floor_6 = Function('Sexpression','<&Youngs_Cols_Floor_6&>./(2*(1+ <&Poisson_Floor_6&>))');

%% Parameter used to define threshold value of the performance function

Xresistance = RandomVariable('Sdistribution','normal','mean',7.5e7,'std',1e7);
Xrvset_resistance = RandomVariableSet('Cmembers',{'Xresistance'},'Xrv',Xresistance); %#ok<SNASGU>

%% Define a Input object that contains all the random quantities and functions
% All the RandomVariableSet objects are added to an Input object. In total,
% 249 random variables have been defined.
Xinp=Input('CXmembers', {Xrvset_resistance,Xrvset_Sections,Xrvset_Youngs,Xrvset_Density,Xrvset_Poisson}, ...
           'CSmembers', {'Xrvset_resistance','Xrvset_Sections','Xrvset_Youngs', 'Xrvset_Density', 'Xrvset_Poisson'}, ...
           'Lcheckfunctions',false);

Xinp = Xinp.add(Shear_Cols_Floor_1);
Xinp = Xinp.add(Shear_Cols_Floor_2);
Xinp = Xinp.add(Shear_Cols_Floor_3);
Xinp = Xinp.add(Shear_Cols_Floor_4);
Xinp = Xinp.add(Shear_Cols_Floor_5);
Xinp = Xinp.add(Shear_Cols_Floor_6);
